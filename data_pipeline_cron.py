...
# Set The Command To Run The Data Pipeline script and activate the virtual environment
    if cronDir is not None:
        job = cron.new(command='cd {0} && env/bin/python  data_pipeline.py {1}'.format(cronDir,mode))
    else:
...
# Write the Job To CronTab
    ...
    logging.info('After Execution Please Check in /var/log/data_pipeline.log For Logs')
